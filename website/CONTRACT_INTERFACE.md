...
| `submit_seller_response` | Accept policy, submit UTF-8 evidence, fully fund purchase escrow | Exactly `purchase_amount_wei` |
...
All refund and mutual-resolution percentages use the immutable
`LOCKED_PURCHASE_AMOUNT_ESCROW` basis. `PRODUCT_PHOTO` is not an accepted type;
fetched evidence must be strict UTF-8 text.
