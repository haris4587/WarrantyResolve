...
const CONTRACT_ADDRESS = import.meta.env.VITE_WARRANTY_RESOLVE_ADDRESS ||
  "0x9997c4E5478893b90a38EB28dEcE57e409012e2f";
...
const SAMPLE_MANIFEST = [
  ...
  "PRODUCT_CONDITION_REPORT|https://example.com/product-condition.txt|<64-char-sha256>",
].join("\n");
...
function App() {
  ...
  async function loadClaim(id) {
    ...
    setError("");
    try {
      const claimRaw = await readContract("get_claim", [id]);
      ...
      const [customerRaw, sellerRaw, judgmentRaw, appealRaw, resolutionRaw] = await Promise.all([
        ...
        claim.open_resolution_id
          ? readContract("get_resolution", [claim.open_resolution_id])
          : Promise.resolve(""),
      ]);
      const detail = {
        claim,
        ...
        resolution: parseJson(resolutionRaw),
      };
      setSelectedId(id);
      setSelected(detail);
      ...
      setSellerForm((current) => ({
        ...current,
        claimId: id,
        policyUrl: claim.policy_url,
        policyHash: claim.policy_sha256,
        deposit: fromWei(claim.purchase_amount_wei),
      }));
  ...
  async function handleSellerResponse(event) {
    event.preventDefault();
    try {
      const selectedClaim = selected?.claim?.claim_id === sellerForm.claimId ? selected.claim : null;
      if (selectedClaim && toWei(sellerForm.deposit) !== BigInt(selectedClaim.purchase_amount_wei)) {
        throw new Error("GEN escrow must exactly equal the locked purchase amount.");
      }
      ...
      setError(errorText(sellerError));
    }
  }
...
function ClaimDetail({ selected, onAction, onView }) {
  const { claim, customer, seller, judgment, appeal, resolution } = selected;
  const evidenceReady = Boolean(customer && seller);
  return <div className="claim-detail"><div className="detail-top"><div><p className="kicker">CLAIM RECORD</p><h3>{claim.product_name}</h3><code>{claim.claim_id}</code></div><StatusBadge value={claim.status} /></div><div className="detail-grid"><DetailCell label="Customer" value={short(claim.customer)} mono /><DetailCell label="Seller" value={short(claim.seller)} mono /><DetailCell label="Purchase / escrow basis" value={`${fromWei(claim.purchase_amount_wei)} GEN`} /><DetailCell label="Current decision" value={claim.current_decision || "PENDING"} accent /></div><div className="commitment"><div><span className="detail-label">Locked policy</span><a href={claim.policy_url} target="_blank" rel="noreferrer">{claim.policy_url}</a></div><code>{short(claim.policy_sha256)}</code></div><div className="protocol-timeline"><TimelineItem done label="Claim opened" detail={`Terms ${short(claim.terms_hash)}`} /><TimelineItem done={Boolean(customer)} label="Customer evidence" detail={customer ? `${customer.evidence_version} submission · ${short(customer.evidence_digest)}` : "Waiting for UTF-8 receipt and condition records"} /><TimelineItem done={Boolean(seller)} label="Seller response + exact escrow" detail={seller ? `${fromWei(claim.escrow_deposited_wei)} GEN = locked purchase amount` : `Waiting for exactly ${fromWei(claim.purchase_amount_wei)} GEN`} /><TimelineItem done={Boolean(judgment)} label="GenLayer judgment" detail={judgment ? `${judgment.decision} · score ${judgment.score}` : "Consensus not requested"} /><TimelineItem done={Boolean(appeal)} label="Appeal window" detail={appeal ? `${appeal.appeal_result} · window reopened` : claim.finalize_after_unix ? `Closes ${dateLabel(claim.finalize_after_unix)}` : "Not open"} /><TimelineItem done={resolution?.status === "ACCEPTED_AND_SETTLED"} label="Mutual resolution" detail={resolution ? `${resolution.status} · ${Number(resolution.customer_payout_bps || 0) / 100}% of purchase escrow` : "No proposal"} /></div><div className="detail-actions">{!customer && <button className="button button-light" onClick={() => onView("customer")}>Add customer evidence</button>}{!seller && <button className="button button-light" onClick={() => onView("seller")}>Add seller response</button>}{evidenceReady && !judgment && <button className="button button-primary" onClick={() => onAction("judge_claim", [claim.claim_id])}>Request judgment</button>}{claim.status === "EVIDENCE_REVIEW" && <button className="button button-light" onClick={() => onAction("retry_judgment", [claim.claim_id])}>Retry evidence</button>}{(claim.status === "JUDGED" || claim.status === "APPEALED") && <button className="button button-primary" onClick={() => onView("appeal")}>Appeal or settle</button>}{claim.status === "OPEN" && <button className="button button-quiet" onClick={() => onAction("cancel_claim", [claim.claim_id])}>Cancel claim</button>}</div><p className="detail-footnote">Every payout percentage is applied to the immutable purchase amount, which the seller must escrow exactly. Evidence is limited to strict UTF-8 text records the contract can evaluate.</p></div>;
}
...
function SettlementCard({ selected, busy, onAction }) {
  ...
  return <div><div className="subsection-heading"><p className="kicker">ESCROW STATE</p><h3>{fromWei(claim.escrow_remaining_wei)} GEN held</h3></div><div className="settlement-state"><div className="settlement-row"><span>Locked purchase basis</span><strong>{fromWei(claim.purchase_amount_wei)} GEN</strong></div><div className="settlement-row"><span>Current decision</span><strong>{claim.current_decision}</strong></div><div className="settlement-row"><span>Customer share</span><strong>{Number(claim.current_refund_bps || 0) / 100}%</strong></div><div className="settlement-row"><span>Outcome binding</span><strong title={claim.current_outcome_binding_digest}>{short(claim.current_outcome_binding_digest || "unbound")}</strong></div><div className="settlement-row"><span>Window</span><strong>{claim.finalize_after_unix ? dateLabel(claim.finalize_after_unix) : "Timeout protected"}</strong></div><div className="settlement-row"><span>Action</span><strong>{claim.settlement_action}</strong></div></div>{appeal && <div className="appeal-record"><span>Latest appeal: {appeal.appeal_result} · {Number(appeal.revised_refund_bps || 0) / 100}% of {fromWei(appeal.payout_basis_wei)} GEN</span><small>{appeal.summary}</small><small title={appeal.appeal_binding_digest}>Binding: {short(appeal.appeal_binding_digest || "unbound")}</small></div>}<button className="button button-primary full-width" disabled={!closable || busy} onClick={() => onAction("release_refund", [claim.claim_id])}>{busy ? "Waiting for finality…" : "Release finalized outcome"}</button><p className="small-note">The contract rechecks the bound decision, percentage, and locked purchase-amount basis before transferring the customer share and seller remainder.</p></div>;
}

function HowItWorks() {
  const steps = [["01", "Open immutable claim", "Customer commits parties, purchase amount, dates, warranty policy digest, and bounded deadlines."], ["02", "Bind text evidence + escrow", "Parties submit hashed UTF-8 records. Seller escrows GEN exactly equal to the locked purchase amount."], ["03", "Run GenLayer judgment", "Validators independently fetch the same bytes and bind the decision, refund percentage, and monetary basis."], ["04", "Challenge or agree", "An appeal can revise the bound percentage; the counterparty can separately accept a mutual-resolution proposal."], ["05", "Release or timeout", "Settlement rechecks the bound outcome and applies its percentage only to the exact purchase-amount escrow."]];
